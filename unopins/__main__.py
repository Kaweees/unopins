...
Support me:
https://github.com/sponsors/kaweees
https://www.patreon.com/kaweees
"""
...
# class Theme(Enum):
#     NC = "bold #9da3a4 on white"
#     IOREF = ""
#     RST = ""
#     TTL = "" # (Transistor-Transistor Logic) Level
#     CMOS = "" # (Complementary Metal-Oxide-Semiconductor) Logic Levels
#     SCK = ""
#     MISO = ""
#     MOSI = ""
#     AREF = ""
...
def usage(error=None):
    ...
    rich.print(
        f"""
[#859900]unopins[/] [#2aa198]v{__version__}[/] - a beautiful GPIO pinout and pin function guide for the Raspberry Pi Pico
{error}
usage: unopins [--...] [--all] or {{{",".join(COLS[2:])}}} [--find <text>]
...
eg:    unopins i2c                    - show GPIO and I2C labels
       unopins                        - basic GPIO pinout
       unopins --all --find "PWM3 A"  - highlight any "PWM3 A" labels
       unopins --all --find "PWM.* A" - highlight any PWM A channels

web:   https://pico.pinout.xyz
bugs:  https://github.com/pinout-xyz/unopins
"""
    )
...
def gpio_style(pin):
    if pin in (3, 8, 13, 18, 23, 28, 38):
        return "ground"
    if pin in (40, 39, 37, 36):
        return "power"
    if pin in (35, 34, 33, 32, 31):
        return "adc"
    if pin == 30:
        return "run"
    return "gpio"
...
def build_pins(pins, show_indexes, highlight=None):
    ...
    has_hidden_results = True in [
        index not in show_indexes and value
...
def unopins(opts):
    ...
    if search("GP25 LED", opts.find):
        ...
        DIAGRAM[LED_ROW + 1] = DIAGRAM[LED_ROW + 1].replace(
            "GP25", styled("GP25", "highlight")
        )
...
def main():
    rich.print(unopins(Options(sys.argv)))
